...
class Kinesis(BaseTest):
    ...
    def test_kinesis_analytics_v2_query(self):
        factory = self.record_flight_data("test_kinesis_analytics_v2_query")
        p = self.load_policy(
            {
                "name": "kapp",
                "resource": "kinesis-analytics",
        ...
        resources = p.run()
        print(f"\nRetrieved kinesis analytics v2 resources: {resources}")
